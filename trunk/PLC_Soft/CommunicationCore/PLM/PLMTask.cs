...
{
	public class PLMTask
	{
		private static int[] tableCRC =	{
		...
		public static int CalculateFSC(byte[] data)
		{
			int crc = 0x0000;
			foreach (byte b in data)
			{
				crc = tableCRC[((crc >> 8) ^ b) & 0xff] ^ (crc << 8);
			}

			return crc;
		}

		public static byte[] AddFSC(byte[] data, int FSC)
		{
			byte[] processedData = new byte[data.Length + 2];
			for (int i = 0; i < data.Length; i++)
			{
				processedData[i] = data[i];
			}
			processedData[data.Length] = (byte)(FSC >> 8 & 0x00ff);
